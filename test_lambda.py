...
if __name__ == "__main__":
    ...
    # ── salva CSV ─────────────────────────────────────────────────────────
    with open('tests/test_lambda.csv', 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=csv_rows[0].keys())
        writer.writeheader()
        writer.writerows(csv_rows)
    logger.info("CSV salvato: test_lambda.csv")
    ...
    plt.suptitle(f"Test lambda — baseline: {round(accuracy_baseline*100,2)}%",
                 fontsize=14)
    plt.tight_layout()
    plt.savefig('tests/test_lambda.png', dpi=150)
    plt.show()
    logger.info("Grafico salvato: test_lambda.png")

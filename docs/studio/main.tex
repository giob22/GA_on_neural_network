...
\begin{itemize}
    \item Da $\lambda = 0$ a $\lambda = 0.05$ la \texttt{test\_accuracy}
          è identica e la stessa architettura viene selezionata. La
          penalità c'è ma non è abbastanza forte da distorcere la
          scelta.
...
\begin{tabular}{lccl}
    ...
    \textcolor{white}{\textbf{architettura}} \\
    \midrule
    0.0  & 89.07\%  & 2110  & [(28, leaky\_relu)] \\
    \rowcolor{rowgray}
    0.05 & 89.07\%  & 2110  & [(28, leaky\_relu)] \\
...
\begin{itemize}
    \item Su \emph{Iris} e \emph{Breast Cancer} la baseline batte il GA
          di pochi decimi di punto. Sono problemi semplici, e la baseline
          (con architettura fissa $[(8,\text{leaky\_relu}), (8,\text{leaky\_relu})]$,
          139 parametri) è già abbastanza espressiva. Il GA trova
          architetture leggermente diverse ma non significativamente
          migliori.
    \item Su \emph{Wine} entrambi raggiungono il $100\%$: dataset
          ampiamente risolto.
    \item Su \emph{Digits} il GA batte la baseline di $32$ punti
          percentuali. Su problemi complessi (molte feature, molte classi,
          molti dati) la ricerca automatica dell'architettura porta valore
          enorme.
\end{itemize}
...
non guadagna molto. Su dataset complessi, dove lo spazio delle
architetture utili è grande e non ovvio, la ricerca automatica può fare
una differenza enorme. Questo è anche il motivo per cui la NAS è
particolarmente utile in applicazioni industriali (visione artificiale,
NLP) dove i problemi sono complessi.
\end{concetto}

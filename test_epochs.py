...
# ── salva CSV ─────────────────────────────────────────────────────────
with open('tests/test_epochs.csv', 'w', newline='') as f:
    writer = csv.DictWriter(f, fieldnames=csv_rows[0].keys())
    writer.writeheader()
    writer.writerows(csv_rows)
print("\nCSV salvato: test_epochs.csv")
...
plt.suptitle(f"Test epochs — baseline: {round(accuracy_baseline*100,2)}%",
             fontsize=14)
plt.tight_layout()
plt.savefig('tests/test_epochs.png', dpi=150)
plt.show()
print("Grafico salvato: test_epochs.png")